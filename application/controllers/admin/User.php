...
class User extends Admin_Controller
{
    ...
    public function user_details($id, $active = null)
    {
        ...
        $data['this_month_hours']= $this->this_month_hours($id);

        $data['subview'] = $this->load->view('admin/user/user_details', $data, TRUE);
        $this->load->view('admin/_layout_main', $data);
    }

    public function this_month_hours($user_id, $project = null)
    {
        $result['tasks'] = 0;;
        $result['project'] = 0;;
        if (!empty($project)) {
            ...
            for ($i = 1; $i <= $num; $i++) {
                ...
                $get_total_hours = $this->db->where('user_id', $user_id)->get('tbl_tasks_timer')->result(); // get all attendace by start date and in date
                if (!empty($get_total_hours)) {
                    foreach ($get_total_hours as $v_hours) {
                        $date = date('Y-m-d', strtotime($v_hours->date_timed));
                        if ($date == $sdate && $v_hours->task_id != 0) {
                            $result['tasks'] += ($v_hours->end_time - $v_hours->start_time);
                        }
                        if ($date == $sdate && $v_hours->task_id == 0) {
                            $result['project'] += ($v_hours->end_time - $v_hours->start_time);
                        }
