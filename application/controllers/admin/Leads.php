...
class Leads extends Admin_Controller
{
        ...
        if (!empty($id)) {
            if ($id == 'kanban') {
                ...
                if ($lead_status_id == 'all') {
                    ...
                    $search = true;
                    $by_status = $this->db->where('lead_status_id', $lead_status_id)->get('tbl_leads')->result();
                }
                $this->session->unset_userdata('leads_kanban');
            } elseif ($id == 'by_source') {
                $data['active'] = 1;
                ...
                if ($lead_source_id == 'all') {
                    ...
                    $search = true;
                    $by_status = $this->db->where('lead_source_id', $lead_source_id)->get('tbl_leads')->result();
                }
                $this->session->unset_userdata('leads_kanban');
            } else {
                $data['active'] = 2;
    ...
    function saved_leads($id = NULL)
    {
        ...
        $data = $this->items_model->array_from_post(array('client_id', 'lead_name', 'lead_status_id', 'lead_source_id', 'mobile',));
        ...
        $message = $msg;
        set_message($type, $message);
        redirect('/admin/leads#workinprogress');
    }
