...
$all_bugs = $this->admin_model->get_permission('tbl_bug');
$today_bugs = 0;
if (!empty($all_bugs)) {
    foreach ($all_bugs as $bug) {
        $bugs_time = date('d-m-y', strtotime($bug->created_time));
        if ($bugs_time == date('d-m-y')) {
            $today_bugs += count($bug);
        }
    }
}
...
if (!empty($all_tickets)) {
    foreach ($all_tickets as $tickets) {
        $tickets_time = date('d-m-y', strtotime($tickets->created));
        if ($tickets_time == date('d-m-y')) {
...
if (!empty($all_task)):
    foreach ($all_task as $task):
        $task_time = date('d-m-y', strtotime($task->task_created_date));
        if ($task_time == date('d-m-y')) {
...
if (!empty($all_project)) {
    foreach ($all_project as $v_project) {
        $project_time = date('d-m-y', strtotime($v_project->end_date));
        if ($project_time == date('d-m-y')) {
            $project_today += count($v_project);
        }
    }
}
?>
                            ...
                            <div class="row row-table row-flush">
                                ...
                                <div class="col-xs-10">
                                    <div class="text-center">
                                        <h4 class="mt-sm mb0"><?php
                                            if (!empty($invoce_total)) {
                                                if (!empty($invoce_total['paid'])) {
                                                    ...
                                                    echo $curency->symbol . ' ' . number_format($paid, 2);
                                                } else {
                                                    echo '0.00';
                                                }
                                            } else {
                                        ...
                                        <small><a href="<?= base_url() ?>admin/invoice/all_payments" class="mt0 mb0">More
                                                info <i
                                        ...
                                        <h4 class="mt-sm mb0"> <?php
                                            if (!empty($invoce_total)) {
                                                ...
                                                if (!empty($invoce_total['due'])) {
                                                    ...
                                                    echo $curency->symbol . ' ' . number_format($total_due, 2);
                                                } else {
                                                    echo '0.00';
                                                }
                                            } else {
                            ...
                            <div class="row row-table row-flush">
                                ...
                                <div class="col-xs-10">
                                    <div class="text-center">
                                        ...
                                        <small><a href="<?= base_url() ?>admin/bugs" class="mt0 mb0">More
                                                info <i
                                                    class="fa fa-arrow-circle-right"></i></a></small>
                                    </div>
                                </div>
